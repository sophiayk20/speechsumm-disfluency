...
def generate_restarts_one_speaker(instance_id, dialogue_dict, mode=Literal['ATAS', 'OTAS', 'ATOS', 'OTOS']):
    ...
    BASE_FOLDER = "/content/drive/MyDrive/ling384/restarts/one_speaker"
    ...
    for person1_turn, person2_turn in zip_longest(dialogue_dict[person_ids[0]], dialogue_dict[person_ids[1]]):
        person_index = random.choice([0, 1]) # disfluent person index
...
for MODE in ['ATOS', 'OTOS']:
    ...
    for instance_id in tqdm(speaker_monologues.keys()):
        generate_restarts_one_speaker(instance_id, speaker_monologues[instance_id], mode=MODE)
    ...
    for instance in tqdm(dataset):
        ...
        with open(f"/content/drive/MyDrive/ling384/restarts/one_speaker/{MODE}-output/{instance['id']}.txt", 'r') as f:
            lines = f.readlines()
    ...
    dataset.push_to_hub("sophiayk20/restarts-one-speaker", split=MODE)
